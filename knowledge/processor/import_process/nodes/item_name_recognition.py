...
class ItemNameRecognitionNode(BaseNode):
    ...
    def process(self, state: ImportGraphState) -> ImportGraphState:
        ...
        # 4. 嵌入商品名(嵌入模型：OpenAI:OpenAIEmbeddings dashscope:text_embedding_v1(2/3/4)--->
        # 稠密向量：主要作用（语义相似性）、稀疏向量主要作用（关键词相似）【bgem3】)
    ...
    def _recognition_item_name_by_llm(self, file_title: str, item_name_context: str) -> str:
        ...
        with PerformanceTimer("LLM商品名识别"):
            ...
            # 整个 try 块是串行阻塞的：调用 LLM → 等返回 → 取 content → 判断 → 返回。
            # 如果是在 Web 服务（如 FastAPI）里这么写，会拖慢整个接口，高并发下容易超时。
            # ainvoke()：异步（async），需要 await，不阻塞线程
            try:
                # 添加重试机制处理速率限制
    ...
    def _embedding_item_name(self, item_name: str) -> Optional[Tuple[list, dict[Any, Any]]]:
        ...
        try:
            ...
            # 3. 获取稠密和稀疏向量
            """
            embedding_result['dense']是二维向量[]
            """
            dense = embedding_result['dense'][0].tolist()

            # 用 indptr 定位 → 取出 indices（编号）和 data（权重）→ 拼成字典
            """
            CSR 格式的稀疏矩阵，它长得像这样：
            sparse = {
                "indptr": [0, 5],  # 索引指针：告诉我们从哪开始、到哪结束
                "indices": [10, 23, 45, 67, 89],  # token编号
                "data": [0.8, 0.3, 0.5, 0.2, 0.9] # 对应权重
            }
            1. start_index = embedding_result['sparse'].indptr[0]
                sparse.indptr = 索引指针数组
                你这里只传入了1 个商品名，所以数组只有两个值：[0, N]
                indptr[0] 永远是 0 意思：从第 0 位开始取数据
            2. end_index = embedding_result['sparse'].indptr[1]
                indptr[1] = 这个商品名的稀疏向量总长度 意思：取到第 N 位结束
            3. weights = ...data[start_index:end_index].tolist()
                取出所有权重值
                变成普通列表：[0.8, 0.3, 0.5...]
            4. tokenIds = ...indices[start_index:end_index].tolist()
                取出所有 token 编号
                变成普通列表：[10,23,45...]
            """
    ...
    def _save_to_milvus(self, file_title, item_name, dense_vector, sparse_vector, config):
        ...
        try:
            ...
            # 2.5 添加数据到Milvus:{"insert_count":10,"ids":[10001,10002,10003]}
...
if __name__ == '__main__':
    # 1. 读取chunk.json
    chunk_json_path = r"D:\dev\hak180产品安全手册\auto\chunks.json"
