...
def register_router(app: FastAPI):
    ...
    async def upload_file_endpoint(background_tasks: BackgroundTasks,
                 file: UploadFile = File(...),
                 original_dir: str = Form(None),  # 使用Form而不是普通参数！
                 service: ImportFileService = Depends(get_import_file_service)):
        # 添加调试日志
        print(f"[upload] 接收到文件: {file.filename}")
        print(f"[upload] 接收到的original_dir: '{original_dir}'")
        print(f"[upload] original_dir类型: {type(original_dir)}")

        # 1. 上传文件（本地/minio）
        task_id, file_dir, import_file_path, original_file_path = service.process_upload_file(file, original_dir)

        # 2. 运行后台任务（跑graph的整个流程）
        background_tasks.add_task(service.run_import_graph,
                            task_id, file_dir, import_file_path, original_file_path)
