from dotenv import load_dotenv

from knowledge.front.service.task_service import TaskService
from knowledge.front.utils.paths import get_local_base_dir
from knowledge.processor.import_process.main_graph import graph


load_dotenv()
import os.path
import uuid
from datetime import datetime
import shutil
from typing import Tuple
from fastapi import UploadFile, HTTPException
from knowledge.utils.minio_util import get_minio_client
from knowledge.processor.import_process.state import ImportGraphState

class ImportFileService:
    """

    文件导入的业务类

    1. 保存上传文件到本地
    2. 保存上传文件到MinIO
    3. 运行图谱的所有节点
    """

    def __init__(self, task_service: TaskService):
        self._task_service = task_service

    def get_date_dir(self) -> str:
        return os.path.join(get_local_base_dir(), datetime.now().strftime('%Y%m%d'))

    def save_upload_file_to_local(self, file: UploadFile, file_dir: str):
        """
         将上传的文件保存到本地
        Args:
            file: 上传的文件
            file_dir: 文件的归档目录

        Returns:

        """

        # 1. 确保归档目录存在
        os.makedirs(file_dir, exist_ok=True)

        # 2. 构建上传文件的完整的path
        import_file_path = os.path.join(file_dir, file.filename)

        # 3. 写入(批量的写入shutil.copyfileobj(file.file,f))
        with open(import_file_path, 'wb') as f:
            shutil.copyfileobj(file.file, f)

        # 4. 返回导入文件的path
        return import_file_path

    def save_upload_file_to_minio(self, import_file_path: str, file: UploadFile):
        """

        Args:
            import_file_path:
            file:

        Returns:

        """

        # 1. 获取minio客户端
        minio_client = get_minio_client()

        # 2. 判断minio客户端是否存在
        if not minio_client:
            raise HTTPException(status_code=500, detail="MinIO 服务不可用")

        # 3. 构建Minio客户端对象名（归档文件）
        minio_object_name = f"origin_files/{datetime.now().strftime('%Y%d%m')}/{file.filename}"

        # 4. 获取桶名
        bucket_name = os.getenv("MINIO_BUCKET_NAME")
        # 5. 开始上传
        try:
            minio_client.fput_object(bucket_name, minio_object_name, import_file_path)
        except Exception as e:
            raise ValueError(f"{file.filename}文件上传失败 原因:{e}")

    def process_upload_file(self, file: UploadFile, original_dir: str = None) ->Tuple[str,str,str,str]:
        """
        处理上传文件
        Args:
            file: 上传的文件
            original_dir: 原始文件所在目录（可选），用于查找images文件夹
        Returns:
        1. 标记当前文件上传节点（"upload_file"）为正在运行中
        2. 将上传的文件保存到本地
        3. 将上传的文件保存到minio
        4. 标记当前文件上传节点（"upload_file"）为运行完毕
        5. 需要返回四部分信息（taski_id file_dir import_file_path original_file_path）
        """

        # 1. 构建时间日期的文件目录出来
        date_dir = self.get_date_dir()

        # 2. 生成一个任务id
        task_id = str(uuid.uuid4())

        # 3. 构建文件的最终归属目录
        file_dir = os.path.join(date_dir, task_id)

        self._task_service.mark_node_running(task_id, "upload_file")
        # 4. 将接收到的文件上传本地
        import_file_path = self.save_upload_file_to_local(file, file_dir)

        # 5. 如果提供了原始目录，构建原始文件路径
        original_file_path = ""
        if original_dir:
            original_file_path = os.path.join(original_dir, file.filename)

        # 6. 将本地磁盘的上传文件同步minio中（原始备份，可选）
        # 注释：MinIO 主要用于存储图片，源文件已保存到本地
        # 如需备份原始文件，可以取消下面这行的注释
        # self.save_upload_file_to_minio(import_file_path, file)

        self._task_service.mark_node_done(task_id, "upload_file")

        # 7. 构建返回值
        return task_id, file_dir, import_file_path, original_file_path

    def  run_import_graph(self,task_id:str,file_dir:str,import_file_path:str,original_file_path:str = ""):
        """
        运行导入graph的流程（跑节点）

        1. 构建初始状态
        graph.stream()之前调用update_task_status更新任务的状态为processing
        2. 运行（graph.stream()）
        2.1 在运行图的某个节点之前调用mark_node_running
        2.2 在运行图的某个节点结束调用mark_node_done
        graph.stream()执行完所有节点 update_task_status更新任务的状态为completed
        Args:
            task_id:
            file_dir:
            import_file_path:
            original_file_path: 原始文件路径（可选）

        Returns:

        """
        try:
            # 1. 标记任务开始处理
            self._task_service.update_task_status(task_id, "processing")

            # 添加调试日志
            print(f"[ImportFileService] task_id: {task_id}")
            print(f"[ImportFileService] file_dir: {file_dir}")
            print(f"[ImportFileService] import_file_path: {import_file_path}")
            print(f"[ImportFileService] original_file_path: {original_file_path}")

            # 2. 构建 LangGraph 初始状态
            global_graph_init_status: ImportGraphState = {
                "task_id": task_id,
                "file_dir": file_dir,
                "import_file_path": import_file_path,
                "original_file_path": original_file_path if original_file_path else ""
            }
            print(f"[ImportFileService] 创建的状态中 original_file_path: {global_graph_init_status.get('original_file_path', '')}")

            # 3. 流式执行整个导入流水线
            for event in graph.stream(global_graph_init_status):
                for key, value in event.items():
                    # 标记节点运行完成
                    self._task_service.mark_node_done(task_id, key)
                    print(f"[{task_id}] Completed Node: {key}")

            # 4. 标记任务完成
            self._task_service.update_task_status(task_id, "completed")

        except Exception as e:
            self._task_service.update_task_status(task_id, "failed")
            print(f"[{task_id}] Error: {e}")





